...
// --- Data for the Offering Cards ---
// Using placeholder images. Replace these URLs with your actual image imports.
// e.g., import careerPanelImg from './assets/career_panel.jpg';
const offeringsData = [
  {
    imgUrl: 'https://images.unsplash.com/photo-1556761175-5973dc0f32e7?q=80&w=1932&auto=format&fit=crop',
  ...
  {
    imgUrl: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?q=80&w=1770&auto=format&fit=crop',
  ...
  {
    imgUrl: 'https://images.unsplash.com/photo-1573497491208-6b1acb260507?q=80&w=1770&auto=format&fit=crop',
  ...
  {
    imgUrl: 'https://images.unsplash.com/photo-1523240795612-9a054b0db644?q=80&w=1770&auto=format&fit=crop',
...
const StudentIcon = ({ style }) => (
  <svg style={style} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" fill="currentColor">
...
const StairsIcon = ({ style }) => (
  <svg style={style} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" fill="currentColor">
...
const PinIcon = ({ style }) => (
  <svg style={style} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" fill="currentColor">
...
const Feature = ({ Icon, text }) => {
  const featureStyle = { display: "flex", gap: "6px", alignItems: "center", flex: 1 };
  const iconWrapperStyle = { display: "flex", backgroundColor: "#3b3b3bEE", borderRadius: "5px", height: "20px", width: "20px", alignItems: "center", justifyContent: "center", flexShrink: 0 };
  const iconStyle = { height: "13px", color: "white" };
  const textStyle = { fontSize: "11px", fontWeight: 500, color: "#3b3b3bee", lineHeight: 1 };
  return (
    <div style={featureStyle}>
      <div style={iconWrapperStyle}><Icon style={iconStyle} /></div>
...
// --- Reusable Program Card Component (No MUI) ---
const ProgramCard = ({ imgUrl, title, desc, time, students, grade, location }) => {
  const cardStyle = { display: "flex", alignItems: "center", borderRadius: "16px", backgroundColor: "white", boxShadow: "rgba(149, 157, 165, 0.2) 0px 1px 6px 6px", width: "100%", height: "220px", cursor: "pointer" };
  const imageContainerStyle = { flex: 5, padding: "18px", height: "100%", boxSizing: "border-box" };
  const imageStyle = { backgroundImage: `url(${imgUrl})`, backgroundSize: "cover", backgroundRepeat: "no-repeat", backgroundPosition: "center", borderRadius: "16px", width: "100%", height: "100%" };
  const dividerStyle = { height: "calc(100% - 36px)", width: "1px", backgroundColor: "#E4E1E199" };
  const contentStyle = { flex: 11, display: "flex", flexDirection: "column", padding: "16px", boxSizing: "border-box", height: "100%" };
  const titleStyle = { fontWeight: 600, color: "#3b3b3b", fontSize: "18px" };
  const descStyle = { color: "#3b3b3b", fontSize: "11px", marginTop: "6px", flexGrow: 1, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical" };
  const featureRowStyle = { display: "flex", gap: "8px", width: "90%" };
  return (
    <motion.div style={cardStyle} transition={{ duration: 0.6, type: "spring", bounce: 0.5 }} whileHover={{ y: -5, scale: 1.005, transition: { duration: 0.2 } }}>
      <div style={imageContainerStyle}><div style={imageStyle} /></div>
      <div style={dividerStyle} />
      <div style={contentStyle}>
        <div style={titleStyle}>{title}</div>
        <div style={descStyle}>{desc}</div>
        <div style={{ ...featureRowStyle, marginTop: "16px" }}><Feature Icon={ClockIcon} text={time} /><Feature Icon={StudentIcon} text={students} /></div>
        <div style={{ ...featureRowStyle, marginTop: "12px" }}><Feature Icon={StairsIcon} text={grade} /><Feature Icon={PinIcon} text={location} /></div>
      </div>
    </motion.div>
  );
};
      ...
      >
        ...
        {offeringsData.map((offering, i) => (
          <motion.div
            ...
            style={{
              flex: '0 0 clamp(340px, 90vw, 480px)', // Responsive card width
              scrollSnapAlign: 'start',
            }}
          >
            <ProgramCard {...offering} />
          </motion.div>
        ))}
      </div>
    </div>
